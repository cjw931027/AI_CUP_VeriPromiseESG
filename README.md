...
> 排行榜僅回傳單一加權總分(0.6434421);下表各子任務 macro-F1 為本隊以對齊官方口徑之評分程式
> 於 out-of-fold(OOF,依公司分組之 5 折)上計算所得,本地加權 ≈ 0.6129(分組 OOF 較保守,
> 略低於線上 0.6434421,各任務相對強弱趨勢一致)。排行榜本身不提供各任務細項分數。

| 子任務 | 權重 | macro-F1 (本地 OOF) |
|--------|------|----------|
| promise_status（承諾識別） | 20% | 0.795 |
| verification_timeline（驗證時機,4 類） | 15% | 0.589 |
...
- 開發與除錯過程使用 **Anthropic Claude（Opus 4.8）** 輔助撰寫/除錯程式、釐清評估指標口徑與實驗設計討論。最終所有預測均由本隊自訓模型之確定性推論流程產生,無人工修改預測值。
